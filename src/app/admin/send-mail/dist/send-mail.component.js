"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
exports.__esModule = true;
exports.SendMailComponent = void 0;
var core_1 = require("@angular/core");
var SendMailComponent = /** @class */ (function () {
    function SendMailComponent(activeModal, adminService) {
        this.activeModal = activeModal;
        this.adminService = adminService;
        this.reclamation = {};
        this.d = [];
        this.sended = false;
    }
    SendMailComponent.prototype.ngOnInit = function () {
    };
    SendMailComponent.prototype.onSubmit = function () {
        var _this = this;
        this.adminService.sendMail(this.name, this.reclamation).subscribe(function (data) {
            _this.d = data;
            console.log(data);
        });
        this.sended = true;
    };
    __decorate([
        core_1.Input()
    ], SendMailComponent.prototype, "name");
    SendMailComponent = __decorate([
        core_1.Component({
            selector: 'app-send-mail',
            templateUrl: './send-mail.component.html',
            styleUrls: ['./send-mail.component.css']
        })
    ], SendMailComponent);
    return SendMailComponent;
}());
exports.SendMailComponent = SendMailComponent;
